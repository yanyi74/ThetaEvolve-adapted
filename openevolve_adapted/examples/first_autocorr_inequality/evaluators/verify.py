...
def validate_solution(solution_data: Any) -> Tuple[bool, str]:
    try:
        ...
        if abs(total) < MIN_CHECK_VALUE:
            return False, "Sum of heights is zero; invalid for c1 objective"

        # Check that sum squared is not too small
        if total ** 2 < MIN_CHECK_VALUE:
            return False, f"Sum squared is too small (< {MIN_CHECK_VALUE}); invalid for c1 objective"

        return True, "Valid step heights for C1"
...
def compute_objective_value(solution_data: Any) -> float:
    try:
        ...
        max_conv = float(np.max(conv_full))
        if max_conv > MAX_CHECK_VALUE:
            return DEFAULT_ERROR_RETURN

        sum_heights = float(np.sum(heights))
        sum_squared = sum_heights ** 2

        c1_upper = 2 * n * max_conv / sum_squared

        # Final check for invalid results
        if not np.isfinite(c1_upper):
            return DEFAULT_ERROR_RETURN

        return c1_upper

    except Exception as e:
        print(f"Error computing c1 objective: {e}")
        return DEFAULT_ERROR_RETURN
