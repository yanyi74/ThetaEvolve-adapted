...
from openevolve.utils.code_utils import calculate_edit_distance, check_code_identical
from openevolve.utils.metrics_utils import safe_numeric_average
from openevolve.modular_utils.error_constants import ErrorCodes
from openevolve.utils.performance_utils import timed_operation
...
class Program:
    ...
    # Code hash for duplicate detection
    ...
    def _calculate_code_hash(self) -> str:
        """Calculate standardized hash of the code (without comments)"""
        from openevolve.utils.code_utils import calculate_normalized_hash
        return calculate_normalized_hash(self.code, self.language)
...
class ProgramDatabase:
    ...
    def __init__(self, config: DatabaseConfig):
        ...
        self.old_code_hashes: Set[str] = set()
        ...
        # Island switching - program counter for automatic island rotation
        self._island_program_counter: int = 0
        self._programs_per_island: int = self._calculate_programs_per_island()

        logger.info(
            f"Island switching configured: {config.num_islands} islands, "
            f"switching every {self._programs_per_island} programs"
        )
        ...
        # Thread-safe temporary cache for batch operations
        self.temp_programs: List[Program] = []
        self._temp_lock = threading.Lock()  # Protects temp cache operations
        self._db_lock = threading.Lock()    # Protects main database operations
        ...
        self.historical_best_programs: List[Dict[str, Any]] = []
        self.reinit_ratio: float = 0.0  # Set via SingleTaskEvolvingGym
        self.reinit_count: int = 0

        # Database reinitialization step controls
        self.smallest_restart_step: int = 0  # Minimum restart interval steps
        self.largest_restart_step: int = float('inf')  # Maximum restart interval steps
        self.last_reinit_step: int = 0  # Last reinitialization step
    ...
    def add(
        ...
        """
        Add a program to the database (thread-safe)
        ...
        self.code_hashes.add(program.code_hash)
    ...
    def sample(self, num_inspirations: int = 5) -> Tuple[Program, List[Program]]:
        ...
        Raises:
            RuntimeError: If the database is empty (e.g., after reinitialization)
        """
        # Check if database is empty (e.g., after reinitialization)
        if not self.programs:
            raise RuntimeError(
                "Cannot sample from empty database. "
                "This may happen temporarily after database reinitialization. "
                "Please ensure initial program is loaded first."
            )
    ...
    def save(self, path: Optional[str] = None, iteration: int = 0) -> None:
        ...
        # Save programs based on bulk I/O mode
        if self.config.use_bulk_io:
            # Bulk mode: save all programs to single file
            save_programs_bulk(
                programs=self.programs,
                save_path=save_path,
                prompts_by_program=self.prompts_by_program if self.config.log_prompts else None
            )
        else:
            # Legacy mode: save each program individually
        ...
        metadata = {
            ...
            "historical_best_programs": self.historical_best_programs,
            "reinit_count": getattr(self, 'reinit_count', 0),
            "smallest_restart_step": getattr(self, 'smallest_restart_step', 0),
            "largest_restart_step": getattr(self, 'largest_restart_step', float('inf')),
            "last_reinit_step": getattr(self, 'last_reinit_step', 0),
            "use_bulk_io": self.config.use_bulk_io,
            # Island switching state
            "island_program_counter": self._island_program_counter,
        ...
        logger.info(f"Saved database with {len(self.programs)} programs to {save_path} (bulk_io={self.config.use_bulk_io})")

    @timed_operation("Database Load")
    def load(self, path: str) -> None:
        ...
        if os.path.exists(metadata_path):
            ...
            self.last_iteration = metadata.get("last_iteration") or metadata.get("iteration") or 0
            print(f"[DEBUG] Loading last_iteration: from_last_iteration={metadata.get('last_iteration')}, from_iteration={metadata.get('iteration')}, final={self.last_iteration}")
            ...
            self.historical_best_programs = metadata.get("historical_best_programs", [])
            self.reinit_count = metadata.get("reinit_count", 0)
            self.smallest_restart_step = metadata.get("smallest_restart_step", 0)
            self.largest_restart_step = metadata.get("largest_restart_step", float('inf'))
            self.last_reinit_step = metadata.get("last_reinit_step", 0)
            saved_use_bulk_io = metadata.get("use_bulk_io")

            # Island switching state (restore or initialize)
            self._island_program_counter = metadata.get("island_program_counter", 0)
            self._programs_per_island = metadata.get("programs_per_island", self._calculate_programs_per_island())

            logger.info(f"Loaded database metadata with last_iteration={self.last_iteration}")

        # Auto-detect load format: bulk file vs. individual files
        bulk_file_exists = os.path.exists(os.path.join(path, "programs_bulk.json"))
        individual_dir_exists = os.path.exists(os.path.join(path, "programs"))

        use_bulk_for_load = saved_use_bulk_io if saved_use_bulk_io is not None else bulk_file_exists

        # Load programs based on detected format
        if use_bulk_for_load:
            # Bulk mode
            programs_data = load_programs_bulk(path)
        else:
            # Legacy mode
            programs_data = load_programs_individual(path)

        # Reconstruct Program objects from loaded data
        for program_id, program_dict in programs_data.items():
        ...
        logger.info(
            f"Loaded database with {len(self.programs)} programs from {path} "
            f"(bulk_io={use_bulk_for_load})"
        )
        ...
        for dim in self.config.feature_dimensions:
            if dim == "complexity":
                ...
                if not program.metrics:
                    ...
                    scaled_value = self._scale_feature_value("score", avg_score, program.metrics)
                ...
                scaled_value = self._scale_feature_value(dim, score, program.metrics)
    ...
    def _should_filter_by_quality(self, program: Program) -> bool:
        """
        Check if program should be filtered out due to low quality.
        Only filters when database is full and quality_filter_enabled is True.
        """
        # Only filter if enabled and database is at capacity
        if not self.config.quality_filter_enabled or len(self.programs) < self.config.population_size:
            return False

        # Get new program score
        new_score = program.metrics.get("combined_score", safe_numeric_average(program.metrics))

        # Find worst score in current database
        worst_score = min(
            p.metrics.get("combined_score", safe_numeric_average(p.metrics))
            for p in self.programs.values()
        )

        # Filter if new score is not better than worst
        should_filter = new_score <= worst_score
        if should_filter:
            logger.debug(f"Quality filter: rejecting program with score {new_score:.4f} (worst: {worst_score:.4f})")

        return should_filter

    def add_temp(self, program: Program, iteration: int = None) -> str:
        """Add program to temporary cache (thread-safe)"""
        if iteration is not None:
            program.iteration_found = iteration
            self.last_iteration = max(self.last_iteration, iteration)

        with self._temp_lock:
            self.temp_programs.append(program)

        return program.id

    def update_database_from_temp(self, verbose: bool = True) -> Dict[str, int]:
        """Move programs from temp cache to main database (serial, thread-safe) with island switching"""
        # Print temp score distribution before processing
        if verbose:
            self.print_temp_score_distribution()

        # Get and clear temp cache
        with self._temp_lock:
            temp_programs = self.temp_programs.copy()
            self.temp_programs.clear()

        if not temp_programs:
            return {"added": 0, "filtered_quality": 0, "filtered_duplicate": 0, "lazy_penalty_count": 0,
                    "island_switches": 0, "migration_triggered": False}

        stats = {"added": 0, "filtered_quality": 0, "filtered_duplicate": 0, "lazy_penalty_count": 0,
                 "island_switches": 0, "migration_triggered": False}

        # Lazy penalty scores for verification
        lazy_penalty_scores = {ErrorCodes.NO_VALID_CHANGE}
        print(f"Lazy penalty scores for verification: {lazy_penalty_scores}")
        
        # Count programs with lazy penalty scores in temp cache
        lazy_penalty_count = sum(
            1 for program in temp_programs
            if program.metrics.get("combined_score") in lazy_penalty_scores
        )
        stats["lazy_penalty_count"] = lazy_penalty_count

        if verbose:
            print(f"Updating database from {len(temp_programs)} cached programs...")
            if lazy_penalty_count > 0:
                print(f"  Programs with lazy penalty scores in temp cache: {lazy_penalty_count}")

        # Process serially (add() method already has locks)
        for i, program in enumerate(temp_programs):
            if verbose and i % 20 == 0:
                print(f"  Processing {i+1}/{len(temp_programs)}...")

            original_size = len(self.programs)

            # Check quality filter
            if self._should_filter_by_quality(program):
                stats["filtered_quality"] += 1
                continue

            self.add(program, iteration=program.iteration_found)
            stats["added"] += 1

            # Island management (matches native openevolve_adapted logic in process_parallel.py:462-474)
            # Check if need to switch island BEFORE incrementing counters
            if self._island_program_counter >= self._programs_per_island:
                self.next_island()
                self._island_program_counter = 0
                stats["island_switches"] += 1
                logger.info(f"Island switching: Switched to island {self.current_island}")

            # Increment counters for current island (after potential switch)
            # This counts iterations on the island, not programs added to it
            self._island_program_counter += 1
            self.increment_island_generation()

            # Check migration after island generation increment
            if self.should_migrate():
                # print(f"🔄 [MIGRATION] Triggering migration at island {self.current_island}")
                self.migrate_programs()
                stats["migration_triggered"] = True
                self.log_island_status()

        if verbose:
            print(f"Database update summary: {stats['added']} added, "
                  f"{stats['filtered_duplicate']} duplicates skipped, "
                  f"{stats['filtered_quality']} low-quality filtered")
            print(f"  Lazy penalty verification: {stats['lazy_penalty_count']} programs with lazy penalty scores "
                  f"(expected ≈ duplicates: {stats['filtered_duplicate']})")

            # Debug: Show hash collision statistics if any warnings were logged
            total_hash_checks = len(temp_programs)
            print(f"  Hash collision debugging: {total_hash_checks} programs processed with enhanced duplicate detection")

            # Island switching statistics
            print(f"  Island switching: {stats['island_switches']} switches, "
                  f"migration_triggered: {stats['migration_triggered']}, "
                  f"current_island: {self.current_island}")

            print(f"Database now has {len(self.programs)} programs, temp cache currently have {len(self.temp_programs)} programs.")

        # self.old_code_hashes = self.code_hashes.copy()
        self.init_old_code_hashes()
        return stats

    def init_old_code_hashes(self) -> None:
        """Initialize old_code_hashes snapshot (called after adding initial program)"""
        self.old_code_hashes = self.code_hashes.copy()

    def _check_program_duplicate(self, program: Program, hash_set: Set[str]) -> bool:
        """Check if program duplicates any existing program using code comparison"""
        if program.code_hash not in hash_set:
            return False
        # Hash match found, verify with precise code comparison

        for existing_program in self.programs.values():
            if existing_program.code_hash == program.code_hash:
                if check_code_identical(program.code, existing_program.code, program.language):
                    return True
        return False

    def get_temp_cache_size(self) -> int:
        """Get current temp cache size (thread-safe)"""
        with self._temp_lock:
            return len(self.temp_programs)

    def print_temp_score_distribution(self) -> None:
        """Print score distribution of temp_programs for debugging"""
        with self._temp_lock:
            if not self.temp_programs:
                print("Temp programs: empty")
                return

            scores = [p.metrics.get("combined_score", 0.0) for p in self.temp_programs if p.metrics]
            if not scores:
                print("Temp programs: no scores available")
                return

            print(f"Temp programs score distribution ({len(scores)} total):")

            # Count negative scores (error codes)
            negative_counts = {}
            positive_scores = []

            for score in scores:
                if score < 0:
                    negative_counts[score] = negative_counts.get(score, 0) + 1
                else:
                    positive_scores.append(score)

            # Print negative scores (error codes)
            if negative_counts:
                neg_items = sorted(negative_counts.items())
                neg_summary = ", ".join([f"{score}:{count}" for score, count in neg_items])
                print(f"  Negative: {neg_summary}")
            ...
            if positive_scores:
                ...
                for p in percentiles:
                    idx = int(total_pos * p / 100) - 1
                    if idx >= 0:
                        perc_values.append(f"{p}%:{positive_scores[idx]:.3f}")

                pos_summary = ", ".join(perc_values)
                print(f"  Positive ({total_pos}): min:{positive_scores[0]:.3f}, {pos_summary}, max:{positive_scores[-1]:.3f}")


    # def program_exists(self, program: Program) -> bool:
    #     """Check if an identical program already exists (hash + full code comparison)"""
    #     # First check hash for fast filtering
    #     if program.code_hash not in self.code_hashes:
    #         return False

    #     # # Hash match found, verify with full code comparison to handle potential collisions
    #     # for existing_program in self.programs.values():
    #     #     if (existing_program.code_hash == program.code_hash and
    #     #         existing_program.code == program.code):
    #     #         return True

    #     # Hash collision detected but no actual code match
    #     logger.warning(f"Hash collision detected for program {program.id[:8]}: "
    #                   f"hash {program.code_hash[:16]} exists but code differs")
    #     return False

    # def compare_program_hashes(self, program1: Program, program2: Program) -> bool:
    #     """Compare two programs' hashes for equality"""
    #     return program1.code_hash == program2.code_hash

    def check_lazy_output(self, child_program: Program, parent_program: Optional[Program],
                         lazy_penalty_level: int) -> Optional[float]:
        """
        Check for lazy output and return appropriate penalty score

        Args:
            child_program: Child program to check
            parent_program: Parent program to compare against
            lazy_penalty_level: Penalty level (0=no penalty, 1=check parent, 2=check database)

        Returns:
            Penalty score if lazy output detected, None otherwise
        """
        if lazy_penalty_level == 0:
            return None

        # Level 1: Check if identical to parent program
        if lazy_penalty_level == 1:
            if (child_program.code_hash == parent_program.code_hash and
                child_program.code == parent_program.code):
                logger.info(f"Lazy output detected: child {child_program.id[:8]} identical to parent {parent_program.id[:8]}")
                return ErrorCodes.NO_VALID_CHANGE

        # Level 2: Check if identical to any program in database (old state)
        elif lazy_penalty_level == 2:
            if self._check_program_duplicate(child_program, self.old_code_hashes):
                logger.info(f"Lazy output detected: child {child_program.id[:8]} duplicates existing program")
                return ErrorCodes.NO_VALID_CHANGE

        elif lazy_penalty_level == 3:
            if self._check_program_duplicate(child_program, self.code_hashes):
                logger.info(f"Lazy output detected: child {child_program.id[:8]} duplicates existing program")
                return ErrorCodes.NO_VALID_CHANGE
        ...
        if not current_island_programs:
            ...
            if self.best_program_id and self.best_program_id in self.programs:
                ...
                if self.programs:
                    ...
                    raise RuntimeError("No programs available in database for sampling")
    ...
    def _enforce_population_limit(self, exclude_program_id: Optional[str] = None) -> None:
        ...
        for program in programs_to_remove:
            ...
            # Remove hash from tracking set
            if program_id in self.programs:
                self.code_hashes.discard(self.programs[program_id].code_hash)
    ...
    def _calculate_programs_per_island(self) -> int:
        """
        Calculate how many programs should be added to each island before switching.

        Based on population_size rather than max_iterations (which doesn't exist in RL4Evolve).
        Goal: Ensure each island is visited at least 10 times before database fills up.

        Returns:
            Number of programs per island before switching
        """
        if self.config.num_islands <= 0:
            return 1

        # Use population_size as the reference (database capacity)
        # Divide by (num_islands * 10) to ensure each island gets ~10 rounds
        programs_per_island = max(1, self.config.population_size // (self.config.num_islands * 10))

        return programs_per_island
        ...
        for i, island in enumerate(self.islands):
            ...
            for migrant in migrants:
                for target_island in target_islands:
                    ...
                    # Add hash to tracking set
                    self.code_hashes.add(migrant_copy.code_hash)
    ...
    def _scale_feature_value(self, feature_name: str, value: float, program_metrics: dict = None) -> float:
        """
        ...
        Args:
            ...
            program_metrics: Optional program metrics to check for timeout condition

        Returns:
            Scaled value in range [0, 1]
        """
        # Special handling for timeout programs - return 0.0 for timeout cases
        if program_metrics and program_metrics.get('timeout', 0.0) == 1.0:
            return 0.0
        ...
        if self.feature_scaling_method == "minmax":
            ...
            if max_val == min_val:
                return 0.0  # Changed from 0.5 to 0.0 - when all values are the same, return 0.0
            ...
            values = stats["values"]
            if not values:
                return 0.0  # Changed from 0.5 to 0.0
            ...
            return self._scale_feature_value_minmax(feature_name, value, program_metrics)

    def _scale_feature_value_minmax(self, feature_name: str, value: float, program_metrics: dict = None) -> float:
    ...
    def log_island_status(self) -> None:
        ...
        for stat in stats:
            ...
            best_indicator = f" (best: {island_best_id[:8]}...)" if island_best_id else ""
            print(
    ...
    def _clear_database(self):
        """Clear all database data structures"""
        self.programs.clear()
        self.feature_map.clear()
        self.code_hashes.clear()
        if hasattr(self, 'old_code_hashes'):
            self.old_code_hashes.clear()
        ...
        # Reset island management state
        self.current_island = 0
        self.island_generations = [0] * len(self.islands)
        self.last_migration_generation = 0

        # Reset island switch counter
        self._island_program_counter = 0
        ...
        # Clear diversity cache etc.
        if hasattr(self, 'diversity_cache'):
            self.diversity_cache.clear()
        if hasattr(self, 'performance_history'):
            self.performance_history.clear()
        if hasattr(self, 'diversity_reference_set'):
            self.diversity_reference_set.clear()
        if hasattr(self, 'feature_stats'):
            self.feature_stats.clear()

        # Clear prompt logs
        if hasattr(self, 'prompts_by_program') and self.prompts_by_program is not None:
            self.prompts_by_program.clear()

    async def database_reinit_async(self, initialize_func, initial_program_path: str, current_step: int = None, verbose: bool = True) -> bool:
        """
        Async version of database reinitialization check and execution

        Args:
            initialize_func: Async initialization function (should be awaitable callable)
            initial_program_path: Initial program path
            current_step: Current training step
            verbose: Whether to output detailed information

        Returns:
            bool: Whether reinitialization was performed
        """
        # Use new check function
        should_reinit, reason = self.should_reinitialize_database(current_step)

        if not should_reinit:
            if verbose:
                print(f"[REINIT] No reinit needed: {reason}")
            return False

        # Perform reinitialization
        if verbose:
            print(f"[REINIT] Triggering database reinit: {reason}")

        # Save current best program
        best_program = self.get_best_program()
        if best_program:
            import time
            # Get score directly from best program
            max_score = best_program.metrics.get("combined_score", safe_numeric_average(best_program.metrics))

            best_record = {
                "program_id": best_program.id,
                "code": best_program.code,  # Add complete code
                "language": best_program.language,  # Add language info
                "metrics": best_program.metrics.copy(),  # Add complete metrics
                "score": max_score,
                "reinit_iteration": self.reinit_count,
                "timestamp": time.time(),
                "code_snippet": best_program.code[:200] + "..." if len(best_program.code) > 200 else best_program.code
            }
            self.historical_best_programs.append(best_record)

            if verbose:
                print(f"[REINIT] Saved historical best program: ID={best_program.id}, score={max_score:.6f}")
        ...
        if verbose:
            print(f"[REINIT] Calling async initialize function to reload initial program...")

        try:
            await initialize_func()  # Async re-execute initialization
            self.reinit_count += 1

            if verbose:
                print(f"[REINIT] Database reinit completed successfully (reinit count: {self.reinit_count})")
                print(f"[REINIT] Programs in database after reinit: {len(self.programs)}")
            return True

        except Exception as e:
            if verbose:
                print(f"[REINIT] ERROR during reinit: {e}")
                import traceback
                traceback.print_exc()
            return False

    def should_reinitialize_database(self, current_step: int = None) -> Tuple[bool, str]:
        """
        Check whether database should be reinitialized

        Args:
            current_step: Current training step

        Returns:
            (should_reinit: bool, reason: str)
        """
        # Check if reinitialization is enabled
        if self.reinit_ratio <= 0.0:
            return False, "reinit disabled"

        # Check step limits
        if current_step is not None:
            steps_since_last_reinit = current_step - self.last_reinit_step

            # Check minimum interval
            if steps_since_last_reinit < self.smallest_restart_step:
                return False, f"too soon (step {steps_since_last_reinit} < {self.smallest_restart_step})"

            # Check maximum interval (forced restart)
            if steps_since_last_reinit >= self.largest_restart_step:
                return True, f"forced restart (step {steps_since_last_reinit} >= {self.largest_restart_step})"

        # Check program count
        if len(self.programs) < 2:
            return False, f"insufficient programs ({len(self.programs)} < 2)"

        # Original score diversity check logic
        scores = [
            p.metrics.get("combined_score", safe_numeric_average(p.metrics))
            for p in self.programs.values()
        ]
        ...
        compare_score = sorted_scores[-1] if len(sorted_scores) == 2 else sorted_scores[-2]

        # Avoid division by zero error
        if abs(max_score) < 1e-9:
            return False, f"max_score too close to zero ({max_score})"

        # Calculate relative difference ratio
        score_ratio = abs(max_score - compare_score) / abs(max_score)

        if score_ratio < self.reinit_ratio:
            return True, f"low diversity (ratio {score_ratio:.6f} < {self.reinit_ratio:.6f})"

        return False, f"sufficient diversity (ratio {score_ratio:.6f} >= {self.reinit_ratio:.6f})"

    def get_historical_best_info(self) -> str:
        """Get historical best program information"""
        if not self.historical_best_programs:
            return "No historical best programs recorded"

        latest_best = self.historical_best_programs[-1]
        all_time_best = max(self.historical_best_programs, key=lambda x: x["score"])

        info = f"Historical best programs: {len(self.historical_best_programs)} recorded, "
        info += f"latest: {latest_best['score']:.6f} (ID: {latest_best['program_id'][:8]}), "
        info += f"all-time: {all_time_best['score']:.6f} (ID: {all_time_best['program_id'][:8]})"
